...
function checkSummary(job: AgentJob): string {
  ...
  return `${checks.filter((check) => check.ok).length}/${checks.length} product checks passed`;
}
...
function judgeSummary(job: AgentJob): string {
  ...
  return judge ? `${judge.approved ? "Approved" : "Rejected"}: ${judge.summary}` : "Independent judge has not started";
}
...
export function reviewPipeline(job: AgentJob, activityReady = true): ReviewPipeline {
  ...
  if (["queued", "interrupted", "needs_attention", "failed", "cancelled"].includes(job.status)) {
    ...
    guidance = action?.state === "repairing" || status === "worker_resumed"
  ...
  const checks = review?.ci || [];
  const post = review?.postMergeCi || [];
  const judge = latestJudgeEvidence(job);
  const preparationComplete = !!review?.reviewBaseRef;
  const ciActionBlocked = ["worker_ci", "candidate_ci"].includes(action?.failureClass || "") && action?.state !== "resolved";
  const ciTone: PipelineTone = status === "ci_running" && reviewActive ? "active"
  ...
  const stages: ReviewPipelineStage[] = [
    ...
    { id: "preparation", label: "Coordinator Git preparation", summary: preparationComplete ? "Candidate prepared on target base" : rebaseConflict ? "Rebase conflict recorded" : status === "ci_running" ? "Completion not yet durably confirmed" : "Awaiting coordinator", tone: preparationComplete ? "complete" : rebaseConflict ? "blocked" : "waiting" },
    { id: "ci", label: "Product CI", summary: checkSummary(job), tone: ciTone, at: transitionAt(job, ["ci_running", "ci_failed", "judging"]), durationMs: checks.length ? checks.reduce((sum, check) => sum + check.durationMs, 0) : transitionDuration(job, ["ci_running"], ["ci_failed", "judging"]) },
    { id: "judge", label: "Independent judge", summary: judgeSummary(job), tone: status === "judging" ? reviewActive ? "active" : "waiting" : status === "rejected" || judge?.approved === false ? "failed" : judge?.approved ? "complete" : "waiting", at: transitionAt(job, ["judging", "approved", "rejected"]), durationMs: transitionDuration(job, ["judging"], ["approved", "rejected"]) },
