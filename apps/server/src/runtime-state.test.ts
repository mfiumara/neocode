...
function fixture(root: string, title = "durable"): DurableRuntimeState {
  const job: AgentJob = {
    ...
    review: { hookToken: "hook", status: "ci_running", attempt: 2, targetBranch: "main", reviewBaseRef: "base-2", preparedHandoffRound: 2, ciHandoffRound: 2, activeRetry: { target: "review", startedAt: 2 }, updatedAt: 2, transitions: [], ci: [{ command: "custom-ci", purpose: "product_ci", handoffRound: 2, ok: true, exitCode: 0, durationMs: 5, output: "passed" }] },
...
test("runtime state atomically round-trips all job and session metadata", async () => {
  ...
  try {
    ...
    assert.equal(restored?.jobs[0]?.job.review?.activeRetry?.target, "review");
